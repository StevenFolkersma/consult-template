;;; consult-template.el --- Simple text templates with consult  -*- lexical-binding: t -*-
...
;; Defining templates:
;;
;;   Call `consult-template-define' with an active region to capture the region
...
;;   `consult-templates-file' and reloaded automatically on next startup.
;;
;; Embark integration:
;;
;;   With embark installed, you can bind `consult-template-define' to
...
;; Configuration:
;;
;;   (setq consult-templates-file "~/my-templates.el")  ; custom storage path
...
(defcustom consult-templates '()
  "Alist of (NAME . TEMPLATE) pairs for `consult-template-insert'.
NAME is a short descriptive string.  TEMPLATE is the text to insert."
  :type '(repeat (cons string string))
  :group 'consult-template)

(defcustom consult-templates-file
  (expand-file-name "templates.el" user-emacs-directory)
  "File used to persist templates defined with `consult-template-define'.
The file is loaded on startup (if it exists) and rewritten on every
call to `consult-template-define'."
  :type 'file
  :group 'consult-template)

;;; Persistence

(when (file-exists-p consult-templates-file)
  (load consult-templates-file nil t))

(defun consult--templates-save ()
  "Write `consult-templates' to `consult-templates-file'."
  (with-temp-file consult-templates-file
    (insert ";; Auto-generated by consult-templates.el -*- lexical-binding: t -*-\n")
    (insert "(setq consult-templates '")
    (pp consult-templates (current-buffer))
    (insert ")\n")))

;;; Define command

;;;###autoload
(defun consult-template-define (name body)
  "Add or replace a template in `consult-templates' and persist it.
...
confirmation before overwriting.  The mark is deactivated after capture."
  (interactive
   (let* ((region-text (when (use-region-p)
     ...
     (list name body)))
  (if-let* ((cell (assoc name consult-templates)))
...
(defun consult--template-preview (start end candidates)
      ...
      (lambda (action cand)
        (cond
         ...
         ((and (eq action 'preview) cand)
          ...
          ;; the original candidate from the list to retrieve template--body.
          (let* ((original (car (member cand candidates)))
                 (body (and original (get-text-property 0 'template--body original))))
            (when body
              (unless ov
                       ...
                       (when-let* ((body (get-text-property 0 'template--body cand)))
                         (concat "  "
                                 (truncate-string-to-width
                                  (replace-regexp-in-string "\n" "↵" body)
                                  60 nil nil t))))
           :state (consult--template-preview start start candidates))))
    (when selected
      (insert (get-text-property 0 'template--body selected)))))

(provide 'consult-template)
;;; consult-template.el ends here
